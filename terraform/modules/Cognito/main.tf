...
# Admin group. The Django admin_required decorator checks that the logged-in
# user's cognito_groups contains "EasyCartAdmins", so the dashboard is gated
# by membership of this group rather than a separate user store.
resource "aws_cognito_user_group" "admins" {
  name         = "EasyCartAdmins"
  user_pool_id = aws_cognito_user_pool.this.id
  description  = "Members can access the admin dashboard"
  precedence   = 1
}
